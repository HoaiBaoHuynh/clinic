...
@section('content-header')
    <h1>
        {{ trans('calendars::doctor_calendars.title.doctor_calendars') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li class="active">{{ trans('calendars::doctor_calendars.title.doctor_calendars') }}</li>
    </ol>
@stop

@section('content')
    @include('calendars::admin.doctor_calendars.partials.index-calendar')
@stop
...
@section('shortcuts')
    <dl class="dl-horizontal">
        <dt><code>c</code></dt>
        <dd>{{ trans('calendars::doctor_calendars.title.create doctor_calendar') }}</dd>
    </dl>
@stop
    ...
    <script type="text/javascript">
        $( document ).ready(function() {
            $(document).keypressAction({
                actions: [
                    { key: 'c', route: "<?= route('admin.calendars.doctor_calendar.create') ?>" }
                ]
            });
        });
    </script>
