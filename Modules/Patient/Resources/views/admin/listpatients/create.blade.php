...
@section('content-header')
    <h1>
        {{ trans('patient::listpatients.title.create listpatient') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.patient.listpatient.index') }}">{{ trans('patient::listpatients.title.listpatients') }}</a></li>
        <li class="active">{{ trans('patient::listpatients.title.create listpatient') }}</li>
    </ol>
@stop
<script type="text/javascript">
    function validate() {
        ...
        if(SDT !=='')
        {
            ...
            {
                document.getElementById("SDT").innerHTML ="Số điện thoại của bạn không đúng định dạng!";
                status = false;
            }
            else
            {
        ...
        {
            document.getElementById("SDT").innerHTML = "Bạn chưa điền số điện thoại!";
            status = false;
        }
...
@section('content')
    {!! Form::open(['onsubmit' => 'return validate()','name' => 'myform','route' => ['admin.patient.listpatient.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('patient::admin.listpatients.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
