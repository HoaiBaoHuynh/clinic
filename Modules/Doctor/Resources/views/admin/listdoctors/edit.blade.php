...
@section('content-header')
    <h1>
        {{ trans('doctor::listdoctors.title.edit listdoctor') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.doctor.listdoctor.index') }}">{{ trans('doctor::listdoctors.title.listdoctors') }}</a></li>
        <li class="active">{{ trans('doctor::listdoctors.title.edit listdoctor') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.doctor.listdoctor.update', $listdoctor->id], 'method' => 'put', 'enctype' => 'multipart/form-data']) !!}
    {{ csrf_field() }}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('doctor::admin.listdoctors.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
